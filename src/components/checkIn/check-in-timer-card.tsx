...
export const CheckInTimerCard: React.FC<CheckInTimerCardProps> = React.memo(({ timer, onCheckIn }) => {
  ...
  const progress = timer.DurationMinutes > 0
    ? Math.min((timer.ElapsedMinutes / timer.DurationMinutes) * 100, 100)
    : 0;

  // Use TargetTypeName from API, fall back to translation key lookup
  const typeLabel = timer.TargetTypeName
    || (CHECK_IN_TYPE_KEYS[String(timer.TargetType)] ? t(CHECK_IN_TYPE_KEYS[String(timer.TargetType)]) : String(timer.TargetType));

  const statusKey = `check_in.status_${timer.Status.toLowerCase()}` as const;

  const elapsedText = timer.LastCheckIn
    ? t('check_in.minutes_ago', { count: Math.round(timer.ElapsedMinutes) })
    : '';

  return (
    <Box
      className={`mb-2 rounded-lg border-l-4 p-3 ${colorScheme === 'dark' ? 'bg-neutral-800' : 'bg-white'}`}
          ...
          <HStack className="flex-1 items-center gap-2">
            ...
            <VStack className="ml-2 flex-1">
              <Text className="text-sm font-bold">
                {timer.TargetName || timer.TargetEntityId || typeLabel}
              </Text>
              {timer.TargetName ? (
                <Text className="text-xs font-medium text-gray-500">{typeLabel}</Text>
              ) : (
                <Text className="text-xs font-medium text-gray-500">
                  {timer.TargetTypeName || String(timer.TargetType)}{timer.TargetEntityId ? ` #${timer.TargetEntityId}` : ''}
                </Text>
              )}
            </VStack>
          </HStack>
