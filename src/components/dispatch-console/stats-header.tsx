import { AlertTriangle, Clock, Phone, Radio, Truck, Users } from 'lucide-react-native';
...
export const StatsHeader: React.FC<StatsHeaderProps> = ({ activeCalls, pendingCalls, unitsAvailable, unitsOnScene, personnelOnDuty, currentTime }) => {
  ...
  return (
    <Box className="border-b border-gray-200 bg-white p-2 dark:border-gray-700 dark:bg-gray-900">
      <HStack className="flex-wrap items-center justify-between" space="sm">
        ...
        {/* Units On Scene */}
        <StatItem icon={Radio} label={t('dispatch.units_on_scene')} value={unitsOnScene} color="#3b82f6" darkColor="#60a5fa" bgClassName="bg-info-50 dark:bg-info-950" />
        ...
        <HStack className="flex-1 items-center justify-center rounded-lg bg-gray-100 p-2 dark:bg-gray-800" space="sm">
          ...
          <Text className="text-lg font-bold text-gray-800 dark:text-gray-100">{currentTime}</Text>
        </HStack>
      </HStack>
    </Box>
  );
