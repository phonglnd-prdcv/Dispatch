...
class OfflineEventManager {
  ...
  private initializeAppStateListener(): void {
    ...
    // Offline event processing has different semantics on web
    if (Platform.OS === 'web') {
      logger.info({
        message: 'Offline event manager AppState listener skipped on web platform',
      });
      return;
    }
    ...
    logger.info({
      message: 'Offline event manager AppState listener initialized',
